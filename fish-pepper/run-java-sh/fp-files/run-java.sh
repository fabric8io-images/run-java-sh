...
debug_options() {
  if [ -n "${JAVA_ENABLE_DEBUG:-}" ] || [ -n "${JAVA_DEBUG_ENABLE:-}" ] ||  [ -n "${JAVA_DEBUG:-}" ]; then
    ...
    echo "-agentlib:jdwp=transport=dt_socket,server=y,suspend=${suspend_mode},address=${debug_port}" | awk '$1=$1'
  fi
}
...
# Shorthand formula evaluation to integer
int() {
  local formula="$1"
  shift 1
  echo "$(calc "${formula}" "%d\n" "$@")"
...
calc_mem_opt() {
  ...
  local val=$(int 'round($1*$2/100/1048576)' "${max_mem}" "${fraction}")
...
ci_compiler_count() {
  local core_limit="$1"
  local log_cpu=$(int 'log2($1)' "$core_limit")
  local loglog_cpu=$(int 'log2(max2($1,1))' "$log_cpu")
  local count=$(int 'max2($1*$2,1)*3/2' "$log_cpu" "$loglog_cpu")
  local c1_count=$(int 'max2($1/3,1)' "$count")
  local c2_count=$(int 'max2($1-$2,1)' "$count" "$c1_count")
  [ $(c2_disabled) = true ] && echo "$c1_count" || echo $(int '$1+$2' "$c1_count" "$c2_count")
}
...
get_java_options() {
  ...
  # of the awk idiom
  echo "${JAVA_OPTIONS:-} $(run_java_options) $(debug_options) $(java_opts)" | awk '$1=$1'
}
